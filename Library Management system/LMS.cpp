    ...
    if(file.is_open())
    {
        ...
        while(getline(file,line)){
            ...
            while(getline(ss,item,',')){
                item.erase(0,item.find_first_not_of("\t\n\r")); // trim the leading spaces
                item.erase(0,item.find_last_not_of("\t\n\r")+1); // trim the trailing spaces
            ...
            try{
                switch(col){
                    case 0 : 
                        if(!item.empty()){
                            ...
                            throw invalid_argument("Invalid book ID");
                        }
                    break;
                ...
                cout<<"Error covering items to the integer: "<<item<<endl;
